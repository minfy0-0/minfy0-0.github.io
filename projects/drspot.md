...
저는 YOLOv8 학습·검증과 하이퍼파라미터 튜닝, 모델별 성능·오류 분석을 맡았습니다. 탐지 결과를 분류 모델에 넘기는 추론 흐름과 AWS 배포 구조도 구현했습니다. 최종 발표자료에는 제 역할을 다음과 같이 정리했습니다.
...
데이터 중복 제거와 클래스 불균형 보정은 다른 팀원이 맡았습니다. 아래 데이터 준비 과정은 팀 전체 작업이고, 제가 집중한 부분은 이후 학습·평가와 서비스 연결입니다.
...
학습이 진행되면서 Precision과 mAP는 좋아졌지만, Recall은 약 **0.4 수준에 머물렀습니다.** 병변이 있어도 탐지하지 못하는 경우를 고려해야 했습니다. 그래서 탐지에 실패하면 원본 이미지를 분류기에 전달하도록 했습니다.
...
EfficientNet-B4의 평균 점수가 가장 높았지만, 어떤 질환을 서로 혼동하는지는 별도로 확인해야 했습니다. Confusion Matrix에서 반복되는 오분류를 살펴봤습니다.
